...
static void
ICELIB_FormatHeaderOk(char*       resp_head,
                      char*       resp,
                      const char* content)
{
...
static const char*
ICELIB_IceComponentIdToStr(uint32_t componentId)
{
  switch (componentId)
  {
  case 1: return "RTP";
...
void
ICELIB_FormatValidCheckListsBodyTable(ICELIB_INSTANCE* icelib,
                                      char*            s,
                                      char*            content)
{
  int32_t  j = 0;
  ...
  for (i = 0; i < icelib->numberOfMediaStreams; ++i)
  {
    ...
    /* for all valid pairs */
    for (j = 0;
         ( ( pValidPair = pICELIB_validListIteratorNext(&vlIterator) ) !=
           NULL );
         j++)
    {
      ...
      ICELIB_RowStart(&s[strlen(
                           s)], pValidPair->nominatedPair ? "chartreuse" : "white",
                      false);
      ICELIB_FormatUint32_1(&s[strlen(
                                 s)], i,                  false, true, false);
      ICELIB_FormatUint32_1(&s[strlen(
                                 s)], pValidPair->pairId, false, true, false);
      ICELIB_FormatStr(&s[strlen(s)],
                       ICELIB_toString_CheckListPairState(
                         pValidPair->pairState), false, true, false);
      ICELIB_FormatUint32_1(&s[strlen(
                                 s)], pValidPair->nominatedPair ? 1 : 0, false, true,
                            false);

      /* local cand */
      ICELIB_FormatStr(&s[strlen(s)],
                       ICELIB_IceComponentIdToStr(pValidPair->pLocalCandidate->
                                                  componentid), false, true,
                       false);
      ICELIB_FormatStr(&s[strlen(s)],
                       ICELIBTYPES_ICE_CANDIDATE_TYPE_toString(pValidPair->
                                                               pLocalCandidate->
                                                               type), false, true,
                       false);
      netaddr_toString(&pValidPair->pLocalCandidate->connectionAddr,
                       addr,
                       sizeof(addr),
                       true);
      ...
      /* remote cand */
      ICELIB_FormatStr(&s[strlen(s)],
                       ICELIB_IceComponentIdToStr(pValidPair->pRemoteCandidate->
                                                  componentid), false, true,
                       false);
      ICELIB_FormatStr(&s[strlen(s)],
                       ICELIBTYPES_ICE_CANDIDATE_TYPE_toString(pValidPair->
                                                               ...
                                                               ->type), false, true,
                       false);
      netaddr_toString(&pValidPair->pRemoteCandidate->connectionAddr,
                       addr,
                       sizeof(addr),
                       true);
      ICELIB_FormatStr(&s[strlen(s)], addr, false, true, false);
...
void
ICELIB_FormatIcePage(char* resp,
                     char* resp_head)
{
  /* body */
  ...
  sprintf(&resp[strlen(
                  resp)],
          "<BR><A HREF=\"/DumpAllCheckLists\">DumpAllCheckLists</A>\n");
  sprintf(&resp[strlen(
                  resp)],
          "<BR><A HREF=\"/DumpValidCheckLists\">DumpValidCheckLists</A>\n");
