...
static struct ConncheckCB connChkCB;
                                  ...
                                  StunMsgId              transactionId)
{
  ...
  return ICELIB_Result_None;
}


static ICELIB_INSTANCE* icelib;
...
icelib_setup ()
{
  ...
  iceConfig.dropRflx = false;

  ICELIB_Constructor(icelib, &iceConfig);
...
CTEST(icelib_ce, calculate_priority)
{
  uint32_t priority1;
  uint32_t priority2;
  uint32_t priority3;
...
CTEST(icelib_ce, create_foundation)
{
...
CTEST(icelib_ce, triggereedcheck_queue)
{
  bool                  result;
  unsigned int          i;
  unsigned int          j;
  ...
  ASSERT_TRUE( ICELIB_fifoIsEmpty(&f) );
  ASSERT_TRUE( !ICELIB_fifoIsFull(&f) );
...
/*  */
  ...
  ASSERT_TRUE( ICELIB_fifoPut(&f, ( ICELIB_FIFO_ELEMENT) 999) );
/*  */
...
/*  */
  for (k = 0; k < ICELIB_MAX_FIFO_ELEMENTS / 2; ++k, ++i)
  {
    result = ICELIB_fifoPut(&f, ( ICELIB_FIFO_ELEMENT) i + 1000);
    ASSERT_TRUE(!result);
    ASSERT_TRUE(ICELIB_fifoCount(&f) ==
                (ICELIB_MAX_FIFO_ELEMENTS) / 4 + k + 1);
  }
...
CTEST(icelib_ce, triggereedcheck_queue_ekstra)
{
  bool                           result;
  unsigned int                   i;
  unsigned int                   j;
...
CTEST(icelib_ce, ice_timer)
{
  ...
  ASSERT_TRUE( !ICELIB_timerIsRunning(pTimer0) );
/*  */
/* - First run */
/*  */
...
CTEST(icelib_ce, controlling)
{
  icelib_setup();
  ASSERT_TRUE( ICELIB_Start(icelib, true) );
  ASSERT_TRUE( icelib->numberOfMediaStreams == 1);
  ASSERT_TRUE( icelib->iceControlling);
  ASSERT_TRUE( !icelib->iceControlled);
  icelib_teardown();
}



CTEST(icelib_ce, initialState)
{
  icelib_setup();
...
CTEST(icelib_ce, iceSupportVerified)
{
  icelib_setup();
...
CTEST(icelib_ce, checklistInitialState)
{
  icelib_setup();
  ASSERT_TRUE( ICELIB_Start(icelib, true) );
...
CTEST(icelib_ce, conncheck)
{
  ...
  memset( &connChkCB, 0, sizeof(connChkCB) );
...
CTEST(icelib_ce, conncheck_withIncomming)
{
  /*
   *  connChkCB.destination = destination;
   *  connChkCB.source = source;;
...
CTEST(icelib_ce, compareTransactionId)
{
  StunMsgId id1;
  StunMsgId id2;
  StunMsgId id3;
...
CTEST(icelib_ce, makeTieBreaker)
{
...
CTEST(icelib_ce, makeUserNamePair)
{
  char ufrag1[] = "ufr1";
  char ufrag2[] = "ufr2";
  ...
  ASSERT_TRUE( 0 ==
               strncmp(pair, "--no_ufrags--", ICE_MAX_UFRAG_PAIR_LENGTH) );
...
CTEST(icelib_ce, findCandidates)
{
...
CTEST(icelib_ce, splitUfragPair)
{
...
CTEST(icelib_ce, compareUfragPair)
{
...
CTEST(icelib_ce, createRandomString)
{
  char randomStr[127];

  ICELIB_createRandomString(randomStr, 127);

  ASSERT_TRUE(randomStr[126] == '\0');
}



CTEST(icelib_ce, longToIceChar)
{
  char b64[6];
  long data = 45678944;
...
CTEST(icelib_ce, isEmptyCandidate)
{
...
CTEST(icelib_ce, toStringCheckListState)
{
  ASSERT_TRUE( 0 ==
  ...
  ASSERT_TRUE( 0 ==
               strcmp(ICELIB_toString_CheckListState( (ICELIB_CHECKLIST_STATE)7 ),
                      "--unknown--") );
...
CTEST(icelib_ce, toStringCheckListPairState)
{
  ASSERT_TRUE( 0 ==
               strcmp(ICELIB_toString_CheckListPairState(ICELIB_PAIR_IDLE),
                      "IDLE") );
  ...
  ASSERT_TRUE( 0 ==
               strcmp(ICELIB_toString_CheckListPairState( (ICELIB_PAIR_STATE)10 ),
                      "--unknown--") );
...
CTEST(icelib_ce, removeChecksFromCheckList)
{
...
CTEST(icelib_ce, resetCandidate)
{
  ICE_CANDIDATE candidate;
  ICELIB_resetCandidate(&candidate);
  ASSERT_TRUE( ICELIB_isEmptyCandidate(&candidate) );
...
CTEST(icelib_ce, formPairs_IPv4)
{
  ICELIB_CHECKLIST CheckList;
...
CTEST(icelib_ce, formPairs_IPv6)
{
